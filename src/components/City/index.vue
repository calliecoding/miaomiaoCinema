  ...
  <div class="city_body">
    <div class="city_list">
      <div class="city_hot">
        <h2>热门城市</h2>
        <ul class="clearfix">
          <li v-for="item in hotList" :key="item.cityId" @touchstart = 'handleClick(item.cityId,item.name)'>
            {{ item.name }}
          </li>
        </ul>
      </div>
      <div class="city_sort" ref="city_sort">
        <div v-for="item in cityList" :key="item.index">
          ...
          <ul>
            <li v-for="item in item.list" :key="item.cityId" @touchstart = 'handleClick(item.cityId,item.name)'>
              {{ item.name }}
            </li>
          </ul>
        </div>
...
export default {
  ...
  methods: {
    ...
    handleClick(cityId,name){
        ...
        localStorage.setItem("cityId",cityId);  //存储id

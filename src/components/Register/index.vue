<template>
  <div class="register_body">
    <div class="register_email">
      ...
      <button :disabled="disabled" @touchstart="handleToVerify">
        {{ verifyInfo }}
      </button>
    </div>
    <div>
...
export default {
  ...
  data() {
    return {
      ...
      verify: "",
      verifyInfo: "发送验证码",
      disabled: false, // 控制按钮的禁用样式 + 避免重复点击发送验证码
    };
  },
  methods: {
    handleToVerify() {

      if (this.disabled) return; // true 时，不能重复发送验证码
    ...
    countDown() {
      ...
      var timer = setInterval(() => {
        ...
        if (count === 0) { //倒计时 为 0 时
        
        count = 60;	//重制 定时器
        this.verifyInfo = "发送验证码";
        clearInterval(timer);
        this.disabled = false; //变更状态
      }
    }, 1000)
